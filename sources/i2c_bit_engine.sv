...
module i2c_bit_engine #(
  ...
  // SDA control request from controller:
  // open-drain: drive_low=1 => drive SDA low, else release
  ...
  assign sda_o  = 1'b0;              // never drive high
  assign sda_oe = enable && sda_drive_low;

  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      div_cnt       <= '0;
      scl_q         <= 1'b1;          // idle high
      tick_scl_rise <= 1'b0;
      tick_scl_fall <= 1'b0;
    end else begin
      tick_scl_rise <= 1'b0;
      ...
      if (!enable) begin
        ...
        if (div_cnt == CLK_DIV-1) begin
          div_cnt <= '0;
          scl_q   <= ~scl_q;
          if (~scl_q) tick_scl_rise <= 1'b1; // 0->1
          else        tick_scl_fall <= 1'b1; // 1->0
        end else begin
